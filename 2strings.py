...
def fix_machine(debris, product):
    ...
    for n in range(len(product)) :return "Give me something that's not useless next time."  if debris.find(product[n-1])== -1 else product
    ###
...
def fix_machine(debris, product):
    ...
    n,k = 1,0
    while  n <=len(product):
        b,  n,  k = debris.find(product[k]),  n + 1,  k + 1
        if b==-1:
            break
    return ("Give me something that's not useless next time."if b == -1 else product)
